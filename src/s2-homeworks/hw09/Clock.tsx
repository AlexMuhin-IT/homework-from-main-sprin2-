...
function Clock() {
  ...
  const stringTime = currentTime || <br /> // часы24:минуты:секунды (24:00:00)
  const stringDate = currentDate || <br /> // день.месяц.год (01.02.2022)

  // день недели на английском, месяц на английском (https://learn.javascript.ru/intl#intl-datetimeformat)
  const stringDay = currentDay || <br />
  const stringMonth = currentMonth || <br />
  ...
  return (
    <div className={s.clock}>
      ...
      <div className={s.buttonsContainer}>
        <SuperButton id={"hw9-button-start"} disabled={disable} onClick={start} xType={"default"}>
          start
        </SuperButton>
        <SuperButton id={"hw9-button-stop"} disabled={!disable} onClick={stop} xType={"default"}>
          stop
        </SuperButton>
      </div>
    </div>
